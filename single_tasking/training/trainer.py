...
class Trainer():
    ...
    def compute_loss_single(self, output, targets):
        pred = output
        target = targets[:, 1:2]
        mse = self.mse_loss(pred, target)
        l1 = self.l1_loss(pred, target)
        total_loss = 0.8 * mse + 0.2 * l1
        return total_loss 
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if self.use_amp:
                with autocast():
                    ...
                    loss = self.compute_loss_single(outputs, gt_output)
    ...
    def train(self, epochs): 
        ...
        for epoch in tqdm(range(self.start,epochs)):
            ...
            if val_loss < self.best_val_loss:
                print('************************************************')
                print(f"Saving best model weights for epoch {epoch}")
                print('************************************************')
    ...
    def visualize_results(self, epoch):
        ...
        #For single beamformer output
        gt_img = gt_output.cpu().squeeze()[2]
        ...
        axes[0].imshow(gt_norm.numpy().T, cmap='gray', aspect='auto')
        axes[0].set_title('GT - CAPON')
        axes[0].axis('off')
        pred_img = outputs.cpu().squeeze()
        ...
        axes[1].imshow(pred_norm.detach().numpy().T, cmap='gray', aspect='auto')
        axes[1].set_title('Pred - CAPON')
        axes[1].axis('off')
        
        ##For multiple beamformer outputs
        # for i, task in enumerate(['das', 'fdmas', 'capon']):
